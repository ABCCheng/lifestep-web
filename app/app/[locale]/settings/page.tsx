...
const voiceMenuItemClass =
  "flex min-h-11 w-full cursor-pointer items-center justify-between gap-2.5 rounded-xl bg-transparent px-3.5 py-2.5 text-left text-muted-foreground transition-colors hover:bg-[linear-gradient(90deg,color-mix(in_srgb,var(--primary)_12%,transparent),color-mix(in_srgb,#5cc9a7_6%,transparent))] hover:text-foreground focus-visible:bg-[linear-gradient(90deg,color-mix(in_srgb,var(--primary)_12%,transparent),color-mix(in_srgb,#5cc9a7_6%,transparent))] focus-visible:text-foreground [&_svg]:size-4 [&_svg]:text-primary";
...
function SettingsContent() {
  ...
  const { locale, dictionary, copy, lifeStep, theme, setTheme, voices, setVoice } = useApp();
  ...
  function confirmLocaleChange() {
    ...
    localStorage.setItem("lifestep-reload-locale", nextLocale);
  ...
  return <main className="app-page pb-10"><BackHeader title={title} eyebrow={copy.settings} /><article className="app-narrow-width py-[42px] max-md:pb-7 max-md:pt-2">
  ...
  </article>{pendingLocale ? <Modal onClose={() => setPendingLocale(null)} labelledBy="language-reload-title"><div className="text-center"><Languages className="mx-auto mb-4 size-10 text-primary" /><h2 className="m-0 text-[1.7rem]" id="language-reload-title">{lifeStep.journey.reloadTitle}</h2><p className="my-3 leading-6 text-muted-foreground">{lifeStep.journey.reloadBody}</p><button className={cn(primaryActionClass, "w-full")} onClick={confirmLocaleChange}>{lifeStep.journey.confirmReload}</button></div></Modal> : null}</main>;
}
