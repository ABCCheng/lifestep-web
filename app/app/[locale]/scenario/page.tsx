      ...
      {complete ? (
        <Modal onClose={() => setComplete(false)} labelledBy="complete-title">
          <div className="complete-modal">
            ...
            {nextQuery ? <Link className="button" href={`${localizePath("/scenario", locale)}?${nextQuery.toString()}`}>Continue to next scenario <ArrowRight /></Link> : <Link className="button" href={`${localizePath("/stage", locale)}?stage=${params.get("stage") || "ARRIVE"}&journey=${params.get("journey") || "STUDY"}`}>Choose the next scenario <ArrowRight /></Link>}
            <Link className="text-link" href={localizePath("/", locale)}>Return to journey</Link>
          </div>
        </Modal>
      ) : null}
    </main>
